...
class ObiClothWithGraspingChannel(RFUniverseChannel):
    ...
    def _parse_message(self, msg: IncomingMessage) -> None:
        ...
        assert title == 'ObiClothWithGrasping Info', \
            'The information %s is not for obi cloth with grasping, please check uuid to avoid repeat.' % title
    ...
    def LoadObiClothWithGrasping(self, kwargs: dict) -> None:
        """Load an obi cloth with grasping into the scene based on assetbundles. Actually, what we load is an obi
           solver in current stage, because there's only one obi cloth, so this doesn't matter. We will further fix
           this in the future.
        Args:
            Compulsory:
        ...
        # Action name
        msg.write_string('LoadObiClothWithGrasping')
        # Action arguments
