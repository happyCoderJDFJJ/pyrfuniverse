...
class BouncerEnv(RFUniverseGymWrapper):
    ...
    def __init__(self, executable_file=None):
        ...
        super().__init__(
            executable_file,
            rigidbody_channel_id='d281db7b-e963-4d74-b960-ab3e138b2867',
    ...
    def step(self, a: np.ndarray):
        ...
        while(self.success_channel.data == {} or self.success_channel.data['finish_last_step'] == False):
            self._step()
            if 'success' in self.success_channel.data.keys() and self.success_channel.data['success']:
                success = True
            if self._check_fail():
                success = False
                reward = -1
    ...
    def _check_fail(self):
        ...
        if position[1] < -1:
            return True
        if position[0] < -19 or position[0] > 19 or position[2] < -19 or position[2] > 19:
            return True
    ...
    def reset(self):
        self.t = 0
        self.env_param_channel.set_float_parameter('bouncerPosX', self._generate_random_float(-5, 5))
        self.env_param_channel.set_float_parameter('bouncerPosZ', self._generate_random_float(-5, 5))
        self.env_param_channel.set_float_parameter('targetPosX', self._generate_random_float(-5, 5))
        self.env_param_channel.set_float_parameter('targetPosY', self._generate_random_float(2, 7))
        self.env_param_channel.set_float_parameter('targetPosZ', self._generate_random_float(-5, 5))
...
class BouncerEnvV0(BouncerEnv):
    ...
    def __init__(self):
        super().__init__('/home/haoyuan/workspace/rfuniverse/build/Bouncer/RFUniverse.x86_64')
